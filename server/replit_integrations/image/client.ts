 ...
 */
export async function colorizeImage(imageBase64: string, mimeType: string = "image/jpeg"): Promise<string> {
  const response = await ai.models.generateContent({
    model: "gemini-2.5-flash-image",
    contents: [
      {
        role: "user",
        parts: [
          { text: "Colorize this black and white image. Make it look realistic and high quality. Return ONLY the image." },
          {
            inlineData: {
              mimeType: mimeType,
              data: imageBase64
